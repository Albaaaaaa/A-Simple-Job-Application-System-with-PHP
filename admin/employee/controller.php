...
switch ($action) {
	...
	function doInsert(){
		...
		if ( $_POST['FNAME'] == "" OR $_POST['LNAME'] == ""
			...
			redirect("index.php?view=add");
				...
				if ($maxrow > 0) { 
					# code... 
					message("ID karyawan sudah digunakan!", "error");
					redirect("index.php?view=add");
				}else{
					...
					message("Karyawan baru berhasil dibuat!", "success");
					redirect("index.php");
			...
			$age = date_diff(date_create($birthdate),date_create('today'))->y;
		 	if ($age < 20 ){
		       ...
		       redirect("index.php?view=edit&id=".$_POST['EMPLOYEEID']);
				...
				message("Karyawan telah diperbarui!", "success");
		       redirect("index.php?view=edit&id=".$_POST['EMPLOYEEID']);
	    	}
	...
	function doDelete(){
		...
		// }
			message("Karyawan sudah Dihapus!","success");
			redirect('index.php');
		// }
  ...
  function UploadImage(){
	...
	function doupdateimage(){
		...
		if ( $errofile > 0) {
			...
			if ($image_size==FALSE ) {
				message("File yang diunggah bukan gambar!", "error");
				redirect("index.php?view=view&id=". $_GET['id']);
			}else{
